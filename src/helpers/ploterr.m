...
[symbol,logx,logy,hhx,hhy,ax]=getproperties(varargin);
...
if ~isempty(xl) % x errorbars
    ...
    h = plotfct(ax,barx,bary,esymbol); hold on
end
if ~isempty(yl) % y errorbars
    [barx,bary]=barline(x,yl,yh,logx,hhy);
    h = [plotfct(ax,barx,bary,esymbol);h]; hold on
end
...
function [sym,lx,ly,hx,hy,ax] = getproperties(A)
lx=0; ly=0; hx=2/3; hy=2/3; sym='-'; ax = gca; % presets
...
while idx <= n
    ...
    case 'ax'
        if ishandle(val), ax=val;
        else error('Property ax must be a handle to an axis object.');
        end        
     case 'abshhy'
        if isnumeric(val), hy=-abs(val);
